...
public class JitterBuffer {
  ...
  public void put(JitterPacket packet) {
    ...
    if (!this.resetState) {
      for (i = 0; i < this.packets.length; i++) {
        ...
        if (this.packets[i] != null && this.packets[i].data != null) {
          if (this.packets[i].timestamp + this.packets[i].span + this.delayStep
              <= this.pointerTimestamp) {
            this.packets[i] = null;
          }
        }
      }
    ...
    if (this.resetState
        || packet.timestamp + packet.span + this.delayStep >= this.pointerTimestamp) {
      for (i = 0; i < this.packets.length; i++) {
        if (this.packets[i] == null || this.packets[i].data == null) {
          break;
        }
  ...
  public void get(JitterPacket packet) throws Exception {
    ...
    if (this.resetState) {
      ...
      for (i = 0; i < this.packets.length; i++) {
        if (this.packets[i] != null
            ...
            && (!found || this.packets[i].timestamp < oldest_timestamp)) {
    ...
    for (i = 0; i < this.packets.length; i++) {
      if (this.packets[i] != null
          ...
          && this.packets[i].timestamp + this.packets[i].span
    ...
    if (i == this.packets.length) {
      ...
      for (i = 0; i < this.packets.length; i++) {
        if (this.packets[i] != null
            && this.packets[i].data != null
            && this.packets[i].timestamp <= this.pointerTimestamp + this.delayStep
            && this.packets[i].timestamp >= this.pointerTimestamp) {
          if (!found
  ...
  public void debugPrint() {
    ...
    for (int i = 0; i < packets.length; i++) {
      ...
      if (p != null) {
        System.out.printf(
            "Slot %2d | ts: %d | span: %d | seq: %d | len: %d | arrival: %d\n",
